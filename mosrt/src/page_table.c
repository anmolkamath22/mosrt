...
const char *page_table_get_segment_name(uint8_t vpn) {
    ...
    uint32_t heap_start_vpn = VM_SEG_TEXT_PAGES + VM_SEG_RODATA_PAGES + VM_SEG_DATA_PAGES + VM_SEG_BSS_PAGES;
...
uint8_t page_table_get_segment_default_permissions(uint8_t vpn) {
    ...
    if (strcmp(seg, "DATA") == 0 || strcmp(seg, "BSS") == 0 ||
        strcmp(seg, "HEAP") == 0 || strcmp(seg, "STACK") == 0) {
