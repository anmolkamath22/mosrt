...
void pager_proc_init(int pid, void **policy_state) {
    (void)pid;
    if (policy_state != NULL) *policy_state = NULL;
}
...
pager_result_t pager_access_page(int pid, page_table_t *pt, void *policy_state, 
                                 uint8_t vpn, bool write, uint64_t tick, int *out_pfn) {
    (void)policy_state;
    if (pt == NULL) return PAGER_ERROR_ADDR;

    pte_t *pte = page_table_lookup(pt, vpn);
    if (pte == NULL) return PAGER_ERROR_ADDR;
    ...
    if (pte->present) {
        ...
        if (out_pfn != NULL) *out_pfn = pte->frame_num;
        return PAGER_HIT;
    }
    ...
    if (pfn < 0) {
         ...
         * but since Clock scans all frames, the evict handler will look up the specific owner page table. */
        ...
        if (victim_pte != NULL) {
            ...
            if (victim_pte->dirty) {
                int swap_slot = victim_pte->in_swap ? victim_pte->swap_slot : swap_alloc(victim_pid, victim_vpn);
                if (swap_slot < 0) {
